...
class CaseEvidence(TimestampMixin, Base):
    ...
    raw_artifact_id: Mapped[int | None] = mapped_column(ForeignKey("raw_artifacts.id"), index=True)
...
class SourceCandidate(TimestampMixin, Base):
    ...
    access_decision: Mapped[str] = mapped_column(String(30), default="pending", index=True)
    access_decision_reason: Mapped[str | None] = mapped_column(Text)
    access_decided_by: Mapped[str | None] = mapped_column(String(160))
    access_decided_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
