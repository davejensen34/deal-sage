...
def run_view(db, run):
    attempts = db.scalars(select(DiscoveryAttempt).where(DiscoveryAttempt.run_id == run.id).order_by(DiscoveryAttempt.id)).all()
    count = db.scalar(select(func.count(SourceCandidate.id)).where(SourceCandidate.case_id == run.case_id)) or 0
    return {"id": run.id, "case_id": run.case_id, "profile_id": run.profile_id, "plan": run.plan,
...
async def execute_attempt(db, run, settings, *, request_key, expected_revision, actor, retry_last=False, provider=None):
    key = str(UUID(str(request_key)))
    prior = db.scalar(select(DiscoveryAttempt).where(DiscoveryAttempt.run_id == run.id, DiscoveryAttempt.request_key == key))
    if prior:
        db.refresh(run)
    ...
    if provider is None:
        if frozen["key"] == "fixture":
            provider = FixtureSearchProvider([SearchResult(url=f"https://example.test/fictional-discovery/{slot}",
    ...
    claimed = db.execute(update(DiscoveryRun).where(DiscoveryRun.id == run.id,
        DiscoveryRun.revision == expected_revision, DiscoveryRun.status != "running").values(
    ...
    attempt = DiscoveryAttempt(run_id=run.id, request_key=key, slot=slot, actor=actor,
    ...
    def accept_results():
        ...
        accepted = db.execute(update(DiscoveryRun).where(DiscoveryRun.id == run.id,
            DiscoveryRun.revision == claimed_revision, DiscoveryRun.status == "running").values(revision=claimed_revision))
    ...
    try:
        results = await asyncio.wait_for(SearchService(db).execute(run.case_id, provider,
            ...
            accept_results=accept_results), timeout=timeout)
        count = len(results)
    ...
    completed = db.execute(update(DiscoveryRun).where(DiscoveryRun.id == run.id,
        DiscoveryRun.revision == claimed_revision, DiscoveryRun.status == "running").values(
...
def recover(db, run, *, expected_revision, actor=None, user_id=None):
    attempt = db.scalar(select(DiscoveryAttempt).where(DiscoveryAttempt.run_id == run.id,
        DiscoveryAttempt.status == "running").order_by(DiscoveryAttempt.id.desc()))
    ...
    result = db.execute(update(DiscoveryRun).where(DiscoveryRun.id == run.id,
        DiscoveryRun.revision == expected_revision, DiscoveryRun.status == "running").values(
    ...
    if actor is not None:
        db.add(AuditEvent(actor=actor, user_id=user_id, action="discovery_recovered",
            detail=f"Discovery run {run.id}: retained interrupted outcome; no replay or refund."))
