...
// Deployments may point VITE_API_URL at either an origin or an already-prefixed
// reverse-proxy path; normalize once so requests never become `/api/api/...`.
const API_ROOT=API.endsWith('/api')?API:`${API}/api`;
export async function api<T>(path:string, options?:RequestInit):Promise<T>{const response=await fetch(`${API_ROOT}${path}`,{...options,headers:{'Content-Type':'application/json',...options?.headers}});if(!response.ok){const body=await response.json().catch(()=>({detail:'Request failed'}));throw new Error(body.detail||'Request failed')}return response.json()}
