...
export function ReviewerInbox() {
  ...
  return <div className="reviewer-workbench">
    <div className="page-heading"><div><p className="eyebrow">Reviewer workbench</p><h1>Review business transitions</h1><p>Start with what changed. Decide which evidence deserves a closer look.</p></div><Link to="/research">Sources & research</Link></div>
    ...
    {result.isLoading ? <p role="status">Loading research cases…</p> : result.isError ? <section className="panel" role="alert"><h2>Research cases are unavailable</h2><p>{result.error.message}</p><button onClick={()=>result.refetch()}>Try again</button></section> : result.data && <>
      ...
      {!result.data.cases.length ? <section className="panel inbox-empty"><h2>{result.data.total ? 'No cases on this page' : 'No research cases match'}</h2><p>Unresolved business names are welcome here. Clear filters to see other retained research.</p><p>Bounded discovery setup is the next milestone increment. Existing source refreshes remain available in Sources & research.</p><Link to="/candidates">Review existing candidates</Link></section> : <div className="inbox-cards">{result.data.cases.map(c=><article className="panel inbox-card" key={c.id}>
