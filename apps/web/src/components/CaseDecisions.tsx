...
import {DevelopmentForm,DevelopmentHandoff,emptyDevelopment,type Development} from './DevelopmentBrief';

type Decision={brief_version:number;expected_prior_id:number|null;outcome:string;purpose:string;rationale:string;next_action:string;supporting_source_ids:number[];contradicting_source_ids:number[];change_reason:string;development?:Development};
...
export function CaseDecisions({caseId}:{caseId:number}){
  ...
  const [development,setDevelopment]=useState<Development>(emptyDevelopment);
  ...
  const load=useMutation({mutationFn:async()=>{const h=await history.refetch();if(h.error)throw h.error;const b=await api<Brief>(`/research/cases/${caseId}/brief-versions/${version}`);return {b,prior:h.data?.latest?.id??null}},onSuccess:({b,prior})=>{setBrief(b);setPrior(prior);setDevelopment(emptyDevelopment);pending.current=null;setDraft(d=>({...d,supporting_source_ids:[],contradicting_source_ids:[]}))}});
  const save=useMutation({mutationFn:()=>{pending.current ||= {request_key:crypto.randomUUID(),decision:{...draft,brief_version:brief!.version,expected_prior_id:prior,...(draft.outcome==='create_development_brief'?{development}:{})}};return api<Record>(`/research/cases/${caseId}/decisions`,{method:'POST',body:JSON.stringify(pending.current)})},onSuccess:()=>{pending.current=null;setBrief(null);setDevelopment(emptyDevelopment);setDraft(d=>({...d,rationale:'',next_action:'',change_reason:''}));setPage(1);qc.invalidateQueries({queryKey:['case-decisions',caseId]});qc.invalidateQueries({queryKey:['development-brief',caseId]})}});
  ...
  return <section className="panel case-investigation"><h2>Reviewer decision</h2><p>Record what should happen next for your purpose. This does not verify source claims, accept model output, change candidate status or start monitoring, research or communication.</p>
    {history.isError?<p role="alert">Decision history unavailable. <button onClick={()=>history.refetch()}>Reload decisions</button></p>:history.isLoading?<p>Loading decisions…</p>:history.data?.latest?<><h3>Current decision</h3><DecisionRecord record={history.data.latest} caseId={caseId} canExport={canReview}/></>:<p>No workflow decision recorded.</p>}
    {canReview&&<><h3>{history.data?.latest?'Record a replacement decision':'Record a decision'}</h3><p>First save and inspect a case brief in “Case brief versions.” Choose its exact version below; later source changes do not rewrite that reviewed record.</p>
      ...
      {brief&&<form className="discovery-form" onSubmit={e=>{e.preventDefault();save.mutate()}}><h3>Decision basis: version {brief.version} · {brief.content.source_brief.title}</h3><p>Saved by {brief.actor}. Source reports, original model observations and human judgments remain separately inspectable in the saved brief.</p>
        <label>Decision<select value={draft.outcome} disabled={busy} onChange={e=>edit({outcome:e.target.value})}><option value="more_research">More research</option><option value="monitor">Monitor</option><option value="dismiss">Dismiss</option><option value="create_development_brief">Create development brief</option></select></label>
        ...
        {draft.outcome==='create_development_brief'&&<DevelopmentForm value={development} sources={brief.content.sources} disabled={busy} onChange={v=>{pending.current=null;setDevelopment(v)}}/>}
    ...
    <h3>Decision history</h3>{history.data?.items.map(r=><DecisionRecord key={r.id} record={r} caseId={caseId} canExport={canReview}/>)}<nav aria-label="Decision history pages"><button disabled={page===1} onClick={()=>setPage(page-1)}>Previous decisions</button><span>Page {page}</span><button disabled={!history.data?.has_next} onClick={()=>setPage(page+1)}>Next decisions</button></nav>
  </section>;
}
