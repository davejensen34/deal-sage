...
Case creation exposes configuration through the Python service, not an operator UI. The approved completion run exercised the shared 90-day policy on recent source packets, including a planned Texas ownership transfer with unknown completion date; passed planned dates still route to verification. The earlier 0/3 useful result is unchanged and new human judgments remain unmeasured. Milestone 7 closes with these explicit outcomes, without a live-quality pass or new milestone. See [closeout](../../docs/research/milestone7-closeout.md).
