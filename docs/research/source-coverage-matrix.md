...
| Utah | Utah.gov Business Entity List data request | Entity identity plus reported owner, applicant, and registered-agent rows | Bounded 188-entity custom delivery validated at the authorized $5 cost | Names and addresses only; source roles are control candidates, not validated beneficial ownership; observed BUSINFO demographic flags remain raw-only | validated for bounded entity and owner-role candidate evidence |
...
| Texas | Comptroller Open Data: Active Franchise Taxpayers (`9cir-efmm`) and Active Sales Tax Permits | Broad entity/status/address corroboration and refresh candidates | Active Franchise Taxpayers exercised through a bounded public Socrata request at no marginal cost | Tax or permit presence is not ownership; the current NAICS API field is unusually named `_621111` and is contract-fingerprinted | validated for entity/tax corroboration; rejected for ownership |
...
|---|---|---|---|---|
| CO, UT, TX | OfficialObituary public browse API | Documented state-filtered, paginated obituary metadata API; no account required | September 4 bounded check returned only 1 CO, 3 UT, and 2 TX records, with newest reported death dates in January–May 2026; its browse response exposed edit/session capability fields | supplemental candidate only: allowlist sanitation is now proven against a representative contract, but poor coverage and intended-use review still preclude promotion to a persistent connector |
