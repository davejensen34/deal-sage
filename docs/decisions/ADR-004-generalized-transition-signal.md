...
Issue #129 integrates the catalog with typed case claims, read-only analyst narratives, and the existing reviewed-proposal promotion service. It preserves unresolved cases rather than fabricating owners for entity events, retains unknown activity through a nullable relationship field, and snapshots the policy and event semantics in the promotion audit. Existing historical claims and scores are not recalculated. These are offline-validated research contracts, not live source coverage or calibrated intelligence.
