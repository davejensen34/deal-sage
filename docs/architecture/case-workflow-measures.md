...
## Ledger coverage inventory (#186)

The existing reservations are now accompanied by seven separate inventories: search query cost gaps, recorded model-proposal estimates and missing usage, extraction attempts without proposals, unpriced legacy candidate AI executions, recorded source-refresh costs, document-retrieval fee authorizations and legacy research steps. Follow-up-linked steps are excluded from the legacy step ledger because they mirror reservations. Other potential overlap is disclosed, not summed. The response deliberately has no grand total.

Counts expose missing or incomplete monetary data; neither a zero-default cost nor an unfinished attempt establishes free execution. Retrieval fees describe frozen policy rather than measured network/infrastructure expense. Historical token counts alone cannot reconstruct historical tariffs. External evaluation ledgers, manual files, infrastructure and unrecorded legacy acquisition costs remain outside this database report. This completes M8.5 cost visibility across persisted ledgers, not invoice reconciliation or complete measurement of all expenditure.

Empty inventory, unknown real-search/legacy-model usage and follow-up overlap regressions pass. Desktop/390px rendering shows units, missing counts and the non-additive boundary. Full validation: 559 backend/API tests, 59 frontend tests and TypeScript/build passed (existing chunk warning). No live calls or migration. The M8.6 cohort still needs its own frozen cost/attempt ledger and actual human judgments under Issue #187.
