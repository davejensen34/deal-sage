...
### Attributed temporal and geographic context

`observation_context.py` adds `m7-observation-context-v1`, an offline interpretation over an already valid v2 observation. It preserves the original model payload and carries reviewer identity, interpretation kind, cited source IDs, operating assertion date, and geographic basis/state separately. Interpretation is not a human decision or verified source truth. Invalid context, future dates and unsupported citations fail with fixed errors; invalid model output cannot be repaired by this layer.

No freshness interval is invented. A supported date equal to the assessment date permits the model's operating label to participate in deterministic evaluation; earlier evidence is historical and undated evidence is undated. Both yield unknown assessment-date operations, preserving historical active/inactive labels verbatim. A publication or retrieval date must not be supplied as the supported date unless the evidence actually describes operations at that time. Planned future events are not completed events. This conservative evaluation policy does not change application promotion rules or configure a product freshness SLA.

Requested-state fit is explicitly operating relevance in the cited context, not current statewide coverage or domicile. Cited operating presence in the requested state supports relevance; an explicit cited absence in that state supports exclusion. A different-state operating presence, any address, dateline or legal domicile, or missing evidence yields unknown. This does not assert that the company has no operations elsewhere. Unknown dimensions still retain all useful summaries and questions. Effective dimensions feed the existing deterministic precedence; original model observations remain unchanged, human decisions remain null, and no promotion runs.

`python -m scripts.review_milestone7_observations` binds the retained v3 result and preparation-v2 bundle by exact hashes. Its reviewed context is explicitly attributed to the coding agent and its output is created exclusively, never overwritten. The command has no credentials, provider, database or live mode. It is an offline evaluation report, not a new model request version or an analyst-accepted application record.

### Browser-local human usefulness review

The authenticated application route `/research/evaluation` accepts an explicitly selected `m7-analyst-review-package-v1` file. `review_milestone7_observations --format review-package` builds it only from the previously pinned result and bundle, projecting source text, model dimensions/summary/questions/contradictions, and separately attributed contextual assessment. Preflight expectations and provider instructions are excluded. The page renders imported evidence as text and allows only HTTP(S) source links without embedded credentials; it neither automatically fetches links nor uploads the file. Shape validation and its 2 MB/20-item bounds protect presentation; they are not evidence authentication.

The browser computes SHA-256 over the exact selected file bytes. Exported `m7-human-usefulness-v1` records bind that fingerprint plus recorded input hashes to explicitly selected useful/not-useful/defer judgments, rationale and optional self-reported review seconds. Unreviewed slots remain listed separately. Attribution is self-reported, not a cryptographic identity claim; analyst acceptance and time saved remain null. There is no model-proposal acceptance, promotion, API mutation, automatic inference of time saved, or server persistence. Feedback downloads must be retained separately with the package. Navigating away or selecting another package discards unsaved feedback, as disclosed in the UI. Imports and exports require deliberate user actions; no default judgment is synthesized.

This integrates the evaluation into a usable analyst review surface without importing the historical experiment into live case records. A future feedback-ingestion/aggregation path must validate package fingerprints and distinguish self-reported human feedback from test fixtures before using it in milestone metrics.

### Offline feedback validation and aggregation

`usefulness_feedback.py` and `python -m scripts.summarize_milestone7_feedback` validate explicitly named files against the exact retained analyst package SHA-256. The package's recorded result/request hashes must also match each export. Strict fields, duplicate JSON key rejection, bounded input sizes, timezone-bearing timestamps, exact reviewed/unreviewed slot partitioning and optional bounded integer review seconds prevent malformed records from becoming metrics. Unknown fields, inferred acceptance/time saved, promotions and unrelated fixture packages are refused. Errors exclude raw values and reviewer rationale. The CLI reads named files only and creates output exclusively; no browser state, downloads directory, credentials or application database is accessed.

Reviewer labels normalize Unicode, whitespace and case to catch accidental duplicate reviewer-slot judgments. Overlapping revisions are rejected, not silently resolved by timestamp. Select the intended export explicitly; disjoint partial exports from the same reviewer can be combined. Distinct reviewer labels are not authenticated identities, and the same person using another label cannot be detected. Fingerprint validation establishes file linkage, not proof that a human authored the judgments. Original files remain the audit evidence; the aggregate retains their hashes without copying rationale or reviewer names.

Packet review coverage counts distinct packets with at least one judgment over all three package packets. Judgment usefulness counts useful judgments over all submitted judgments, including defer; multiple reviewers increase the judgment denominator without multiplying packet coverage. Missing duration stays missing; zero is a valid recorded duration. Known duration totals are explicitly partial where values are absent and never imply time saved. No-input summaries retain null usefulness/duration/precision and explicit 0/3 coverage. These outputs do not accept proposals, alter cases or close milestones.

### Human-value failure and discovery remediation

The actual #130 human review rated all three retained packets not useful, primarily for age and secondarily for presentation centered on review instructions. A valid observation contract and contextual assessment are not an intake freshness policy. The frozen M7 queries had no date window; preparation retained publication-age metadata but did not reject stale signals from paid analysis slots; lead priority did not include signal age. The existing confidence multiplier's publication/retrieval fallback is not proof of a recent event. These are observed architectural gaps, not implemented fixes.

Issues #144/#145 now track configurable recent-signal routing before analysis and business-focused briefs in the existing research surface. Event, publication and retrieval dates must remain distinct; old context can support a recent lead without becoming the lead's triggering event. Unknown dates must not be treated as negative ownership evidence. Freshness policy and business value will be validated separately from source authenticity and promotion guards. The negative human result remains part of the permanent evaluation history.
