...
Issue #180 now supplies [saved brief comparisons](brief-versions.md#saved-version-comparisons-180), without completing a review or updating a monitoring date. M8.5 remains open for full explicit-refresh workflow validation and practical review/usefulness/time/cost measures. This increment does not establish that new evidence exists, complete a monitoring question, or schedule provider calls. M8.6 still requires actual human usefulness judgments and the separately frozen live cohort.

Issue #182 supplies [case workflow measures](case-workflow-measures.md) separately from this personal queue. Shared usefulness reports expose no personal monitoring dates or questions and cannot complete/reschedule a reminder. Full refresh validation and complete cost coverage remain open.
