...
Opt-in alerts are evaluated deterministically only after a `SourceRefresh` finishes. The supported event types are `refresh_failed` and `quarantine_detected`; ordinary success is not notification-worthy. Alert subscriptions cannot schedule or invoke acquisition, and delivery is limited to the authenticated analyst's in-app inbox. Disabling a subscription preserves earlier alert history.
