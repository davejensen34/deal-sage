...
Dynamic retrieval is a second explicit boundary. A candidate requires an attributable access decision, and deterministic checks reject pending or blocked decisions, observed authentication/paywall/CAPTCHA/robots/reuse restrictions, private or special network addresses, unsupported media, redirects beyond the limit, oversized responses, and exhausted case document budgets. Successful bytes enter content-addressed `RawArtifact` storage first; `CaseEvidence.raw_artifact_id`, URL, and content hash must then agree with that immutable artifact. Search snippets are never landed through this path.

`ResearchLoopService` executes one already-approved frontier action at a time. For model-originated searches it uses the exact approved query and rejects substitutions; analyst-created work requires an explicit query. Search and retrieval failures retain only a safe exception class. A step resolves its question only when it adds a new candidate or evidence item. Repeated URLs or content remain linked to their acquisition/step history without duplicating candidates or evidence; exhausted attempts then produce the planner's explicit terminal reason.
